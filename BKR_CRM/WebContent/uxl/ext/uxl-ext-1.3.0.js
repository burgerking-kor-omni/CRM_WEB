 ...
 * 
 * File : uxl-ext-1.3.0.js
 * Version 1.3.0
 * Release List
 * 
 * 1.3.0 - 2017-08-08 :
 *       1) 첨부파일 신규 추가시 내부큐 갯수를 늘려주던 로직 제외 처리
 *       2) options.attachType 으로 지정하도록 옵션 추가
 * 1.2.0 - 2017-04-14 : 
...
(function($, uxl) {
	...
	uxl.ext.Upload = function(id, options){
		...
		if(uxl.isEmpty(this.attachType)){
			if(uxl.isEmpty(options.attachType)){
				this.attachType = id;
			}else{
				this.attachType = options.attachType;
			}
		}
			...
			for (var i=0; i<fileDataList.length; i++) {
				...
				if(row.attachType == this.attachType){
					...
					var $fileItem = makeFileItem(row, this.options.readOnly, function() {
						...
						// 2018-08-08 : fileQueue 체크로직 제외 처리 : 외부 파일로딩시 제한 건수 체크 오류
