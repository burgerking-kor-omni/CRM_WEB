...
;(function($, uxl) {
	...
	$(document).ready(function() {
		...
		// 달력 버튼 설정
		uxl.setCalandarButton();

		// 서브 타이틀 접기 기능
		uxl.setSubTitleHolder();
	...
	uxl.setupFormInput = function(target) {
		...
		$('textarea[autosize=1],textarea[autosize=yes]').each(function(index, em) {
			$(em).height(30);
		...
		$('.ub-page.list .ub-layout.search :input:not(:button)').keypress(function(event) {
			if (event.which == '13') {
				event.preventDefault();

				if(uxl.isNotEmpty($(this).val())) {
					$('#' + uxl.CONSTANT.BUTTON.SEARCH).click();
				}
				else {
					var $searchInputs = $('.ub-page.list .ub-layout.search :input:not(:button)').filter(':visible').filter(':enabled');
		...
		$('.ub-page.detail :input:not(:button):not(textarea)').keypress(function(event) {
			if (event.which == '13') {
				event.preventDefault();

				var $detailInputs = $('.ub-page.detail :input:not(:button):not(textarea)').filter(':visible').filter(':enabled');
	...
	uxl.setInputFormat = function() {
		...
		$('input[metaformat]').each(function(){
			uxl.setInputMask(this, $(this).attr('metaformat'));
		});

		$('input[metaformat=date]').change(function() {
			var val = $(this).val();
			if(uxl.isDate(val)) {
	...
	uxl.setCalandarButton = function() {
		$('.ub-control.input-text.calendar').each(function(){
			if(!this.readOnly){
				$(this).datepicker({
				      changeMonth: true,
				      changeYear: true,
...
//		});
		...
		var folderList = $('.ub-control.title[hfold=1]');
		...
		folderList.each(function(){
			...
			for(var i=0; i<forNames.length; i++){
			    if($('#'+uxl.trim(forNames[i])).size() == 0){
			    	checkFor = false;
			    	break;
			    }else{
			    	forObjects[forObjects.length] = $('#'+uxl.trim(forNames[i]))[0];
			    }
			}
	...
	uxl.setMultiSelect = function(){
		$('.ub-control.select.multiselect').each(function(){
	...
	*/
	...
	uxl.setLinkAPMD = function(){
		...
		if(_SERVER_MODE == 'local'){
			...
			$('body').append(html);
			$('.ub-layout.apmd-panel a.apmd-link').click(function(){
				var param = "?SCREEN_ID=" + _SCREEN_ID + "&call_layout=Popup";
				uxl.openWindow('apmdScreenLinkPop', uxl.getScreenURL('APM0002') + param, {scrollbars:'yes', width:1280, height:800});
			});
			
			// 메세지Debug 버튼 표시
			$('.ub-layout.apmd-panel a.message-debug').click(function(){
				uxl.messageDebugMode();
			});
			
			// 다국어 모드변경
			$('.ub-layout.apmd-panel a.lang-list').click(function(){
	 ...
	 */
	uxl.setSearchAdvancedButton = function(){
		var lytSearchAdvanced = $('.ub-layout.search-advanced');
	 ...
	 * 웹 접근성 관련 함수 추가
	 */	
	uxl.setWebAccessibility = function(){
		$('html').attr('lang', _CURR_LANG_CD);							// html의 기본언어 설정
		$('iframe').attr('title', document.title);						// iframe의 Title지정 - 현재문서의 타이틀로 설정
		
		$('th', $('.ub-control.table.normal')).attr('scope', 'col');	// 일반 테이블의 TH Scope은 col
		$('.ub-control.table.normal').each(function(index){
			// Caption이 없을 경우에 생성함.
