...
*/
					...
					completed = function() {
						if(uxl.isNotNull(success)) {
							if(uxl.isString(success)) {
								...
								try {
									if($(document.body).has('#divLoading').length){
										uxl.removeLodingDiv(); 
									}
									success(result);
								}
	...
	uxl.callFunction = function(url, option) {
		...
		$.ajax({
			...
			async: uxl.isUndefined(option.async) ? false : option.async,
...
//		var $div1 = $("<div id='divLoading' class='ub-control loading' style='background-color:#000;position:absolute;top:0px;left:0px;width:"+width+"px;height:"+height+"px;z-index:99009'></div>");
		var $div1 = $("<div id='divLoading' class='ub-control loading' style='position:absolute;top:0px;left:0px;width:"+width+"px;height:"+height+"px;z-index:99009'></div>");
		var left = width/2-66;
		...
		var img = $('<img>').attr('src', uxl.getContextURL('/page/images/common/loadingbar.gif'));
		
		var $div2 = $("<div style='position:absolute;height:66px;width:66px;top:"+top+"px;left:"+left+"px;z-index:99010'></div>").append(img);
	...
	$(document).ajaxError(function(e, xhr, settings, error) {
		...
		if(error.code == 19){
			uxl.showMessage(uxl.getMessage('message.error.http.500')+"error:19");
		}
		
//		if(uxl.isServiceRequestPath(settings.url) || uxl.isFunctionRequestPath(settings.url) ) {
		if(settings.dataType.toLowerCase() == 'json'){
			if(xhr.status == 401) {
				uxl.showMessage(uxl.getMessage('message.error.http.401'));
				if(opener){
					if(opener.opener){
