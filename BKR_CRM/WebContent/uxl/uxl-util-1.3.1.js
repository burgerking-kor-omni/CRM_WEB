 ...
 * 
 * 1.3.0 - 2017-04-14
 *   - uxl.isUniqueId 함수 추가 
 * 
 * 1.2.0 - 2015-11-27
 *   - uxl.isNotBlank 함수의 CallStack Overflow 발생으로 오류 수정처리
 */
;(function($, uxl) {
	...
	*/
	...
	/**
	 * ID가 현재Document에서 유일한지 체크여부
	 * @param {String} Object id
	 * @return {Boolean} 유일하면 true, 아니거나 존재하지 않으면 false
	 * @see uxl-util-1.3.0.js
	 */
	uxl.isUniqueId = function(id){
		return ($('*[id='+id+']').size() == 1)?true:false;
	};

	/**
	 * ID가 사용된 ID인지 체크하는 함수
	 * @param {String} Object id
	 * @return {Boolean} 존재하면 false, 존재하지 않으면 true;
	 * @see uxl-util-1.3.0.js
	 */
	uxl.isUsedId = function(id){
		return ($('*[id='+id+']').size() != 0)?true:false;
	};
	
	/**
	 * HTML의 특수문자 치환을 정상적인 테스트로 표시해주는 함수
	 * &amp; --> &  
	 */
	uxl.html2text = function(str){
