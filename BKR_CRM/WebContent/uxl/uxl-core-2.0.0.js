 ...
 * @version 2.0.0
 *  
 *  2.0.0 : jquery-1.12.1 / jquery-ui-1.12.1 이상을 지원하는 버젼으로 deprecate 된 함수를 재수정함.
 *  		(jquery의 버전은 버그 테스트 사이틑 통하여 사용가능한 버젼을 패치함. 테스트 사이트 : http://research.insecurelabs.org/jquery/test/)
 *  	- result.getExtendAttributeMap 함수 추가
 *  1.2.5 : 2017-11-08
 *  	- result.getColDataInt 함수 추가
 *  	- result.getMultiColDataInt 함수 추가
 *  	- result.isNotFoundData 함수 추가
 *  	- result.hasMore 함수 추가
 *  	- result.getRowCount 버그수정
 *  1.2.4 : 2017-10-10
 *  	- result.getData() 함수 추가
 *  1.2.3 : 2017-04-17
 *      - 1.2.2 : bug 재수정 
...
(function($) {
	...
	uxl.Result = function(data) {
		 ...
		 * 확장속성맵을 리턴하는 함수
		 */
		this.getExtendAttributeMap =  function(){
			return result['extendAttributeMap'][0];
		}
		...
		this.getRowCount = function(dataListId) {
			...
			if(uxl.isNull(result['dataList'])) {
				...
				var rowCount = 0;
				if(uxl.isNotNull(result['dataList'][dataListId])){
					rowCount = result['dataList'][dataListId].rowCount;
				}
		 ...
		 * 예제)
		 *    result.getColData('DATA_LIST_ID', 'COL1')                = ['10', '20', '30', .... 90]
		 */
		...
		/**
		 * 지정한 컬럼의 row를 Int 배열로 리턴한다.
		 * @param dataListId DataList Id
		 * @param columnName ColumnName
		 * 예제)
		 *    result.getColDataInt('DATA_LIST_ID', 'COL1')              = [10, 20, 30, .... 90]
		 */
		this.getColDataInt = function(dataListId, columnName){
			var rtnValue = [];
			var dataList = this.getDataList(dataListId);
			if(uxl.isNotNull(dataList.rows)){
				for(var i=0; i<dataList.rows.length; i++){
					rtnValue.push(parseInt(dataList.rows[i][columnName], 10));
				}
			}
			return rtnValue;
		};
		 ...
		 *    result.getColsData('DATA_LIST_ID', 'COL1', 'COL2')        = [[10,11], [20, 21] ... [90,91]] 
		 */
		this.getMultiColDataInt = function(dataListId){
			var rtnValue = [];
			var columnNames = arguments;
			var dataList = this.getDataList(dataListId);
			if(uxl.isNotNull(dataList.rows)){
				for(var i=0; i<dataList.rows.length; i++){
				    var pushData = [];
				    for(var j=1; j<columnNames.length; j++){
				        pushData[pushData.length] = parseInt(dataList.rows[i][columnNames[j]], 10);
				    }
					rtnValue.push(pushData);
				}
			}
		...
		/**
		 * 검색된 데이터 존재하는지 여부 체크
		 * @return true-검색된 값이 없음, false-검색된 값이 존재함
		 */
		this.isNotFoundData = function(){
			return (serviceMessage.pageNum == 1 && this.getRowCount() == 0);
		};

		/**
		 * 현재 페이지 이후의 데이터가 존재하는지 여부를 리턴하는 함수
		 * @return 데이터 존재여부
		 */
		this.hasMore = function(){
			return !this.isNotFoundData() && (serviceMessage.pageNum < serviceMessage.pageCount);
		};
...
/**
 * Borwser 기능 Deprecate로 인한 기능 추가
 *  -- UI 라이브러리의 호환성을 위하여 추가됨.
 * @returns
 */
