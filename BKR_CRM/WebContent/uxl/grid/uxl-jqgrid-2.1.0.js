(function($, uxl) {
	...
	uxl.grid= function(element, options) {
		...
		if(uxl.isNotNull(uxl.pageSize) && uxl.isNotNull(uxl.pageNumber)){
			$jqGrid.jqGrid('setGridParam', {rowNum:uxl.pageSize});
	...
	uxl.grid.callFunction = function(gridId, pageNum){
		...
		if(searchForm != null) {
			...
			if($('#'+_KEY_PAGE_BLOCK, $(searchForm)).size() == 0){
				uxl.addHidden(searchForm, _KEY_PAGE_BLOCK, parseInt(pageNum/currPageSize, 10));
			}else{
				$('#'+_KEY_PAGE_BLOCK, $(searchForm)).val(parseInt(pageNum/currPageSize, 10));
			}
