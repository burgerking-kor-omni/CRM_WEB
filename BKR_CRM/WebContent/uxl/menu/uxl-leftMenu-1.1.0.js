 ...
 * 
 * File : uxl-leftMenu-1.1.0
 * Version 1.1.0
 * Release List
 * 
 * 1.0.0 - 2016-10-29
 *  - LeftMenu 초기 개발.
 *  1) isToggleMode - 접힘 펼침 기능 사용 여부
 *  
 * 1.1.0 - 2017-03-15
 *  - leftMenu 옵션 추가
 *  1) isResizeMode - leftMenu에 따라 컨텐츠 영역 사이즈 리사이즈 기능 사용 여부
 *  2) tabCnt - 탭 사용 개수 (즐겨찾기, 메뉴 등 등)
 *  3) isLargeMenu - 대메뉴 표시 여부
 */
...
uxl.leftMenu = function(target, option, callback){
	 ...
	 var $leftToggleFrame = $('div.ub-frame.middle > div.ub-frame.left');
	 
	 var options = {
			 isToggleMode : uxl.isUndefined(option.isToggleMode) ? false : option.isToggleMode
			 ,isResizeMode : uxl.isUndefined(option.isResizeMode) ? false : option.isResizeMode
			 ,tabCnt : uxl.isUndefined(option.tabCnt) ? "0" : option.tabCnt
			 ,isLargeMenu : uxl.isUndefined(option.isLargeMenu) ? false : option.isLargeMenu
	 };
	 ...
	 $topMenu.bind('click', function(event){ //메뉴 영역 클릭시 이벤트 바인딩
		...
		if(lastActiveMenuId != activeMenuId  && !uxl.isUndefined(activeMenuId)){
			...
			/*탭영역 show Hide*/
			if(parseInt(options.tabCnt, 10)>0){
				fnShowTabMenu();
			}
	     	...
	     	/*메뉴영역에 따라 컨텐츠영역 사이즈 조절 옵션*/
	     	if(options.isResizeMode==true){
	     		$leftFrame.addClass('move');
	     	}
	 ...
	 function fnMakeLeftLargeMenu($target, $source){
         ...
         var tabCnt = parseInt(options.tabCnt, 10); // 사용할 탭 수
         var html = "";
         if(tabCnt>0){ // 탭 사용 개수가 0개 이상 일 때
		         html = '<div class="ub-layout lnb-wrap">'
		        	  + ' <div class="side-tab">'	
	       	 	  	  + '  <ul>'
        
	   	 	 for(var i=0; i<tabCnt; i++){ //탭 개수 만큼 영역 생성
	       	     html = html +'<li class="tab0'+(i+1)+(i==0?" on":"")+'">'
	         ...
	         for(var i=0; i<(tabCnt-1); i++){ //탭 개수 만큼 영역 생성
	       	 	 html = html + '<div class="tab-cnt hide"></div>'
	         }	 
			     html = html + '</div>'
         }else{
     ...
     * */
     function fnMakeLeftSmallMenu($target, $source){
 	 ...
 	 /*
 	  * LeftMenu 상단 탭 영역 클릭 시 show, hide
 	  */
 	function fnShowTabMenu(){
 		
 		var $tabDiv = $('div.ub-frame.left > div.ub-layout.lnb-wrap > div.side-tab > ul > li > a'); // 메뉴 상단 탭 영역
 		var $tabMenuDiv = $('div.ub-frame.left > div.ub-layout.lnb-wrap > div.tab-cnt'); // 탭 하단 메뉴 컨텐츠 영역
 		
 		$($tabDiv).click(function(){
 			$tabDiv.closest('li').removeClass('on');
 			$(this).closest('li').addClass('on');
 			
 			var tabOrder = parseInt(this.innerText, 10); // 선택된 탭 순서

 			$tabMenuDiv.removeClass("show");
 			$tabMenuDiv.addClass("hide");
 			$tabMenuDiv.eq(tabOrder).removeClass("hide");
 			$tabMenuDiv.eq(tabOrder).addClass("show");
 		});
