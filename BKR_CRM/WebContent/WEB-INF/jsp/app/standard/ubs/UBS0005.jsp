		...
		<tbody>
			<tr id="layout-1_1">
				...
				<td id="layout-1_1_2" colspan="2">
					<select id="ID_BOARD_TYPE" name="ID_BOARD_TYPE" class="ub-control select " style="width:200px;" size="1" datacid="BOARD_TYPE_CD" metaessential="1"  defaultvalue="<%=JspUtils.getDefaultValue(_dl_BoardManage_getTypeDetail, "ID_BOARD_TYPE") %>" readonly disabled>
					...
					<span class="ub-control button inner icon" id="loadBoardBtn">
					    <a href="#none" title="load"><span></span>타 게시판 설정정보 불러오기</a>
					</span>
				</td>
			</tr>
			<tr id="layout-1_2">
			...
			<tr id="layout-1_6">
				<th id="layout-1_6_10"><label class="ub-control label" >파일 첨부</label></th>
...
<!-- 			<tr id="layout-1_15"> -->
<!-- 				<th id="layout-1_15_30"><label class="ub-control label" >텍스트에디터</label></th> -->
<!-- 				<td id="layout-1_15_31" colspan="2"> -->
...
<!-- 			</tr> -->
			...
			<tr id="layout-1_16">
				<th id="layout-1_16_28"><label class="ub-control label" >조회이력 저장</label></th>
				<td id="layout-1_16_29" colspan="2">
					<input type="radio" class="ub-control radio" id="FG_INQUIRY_HISTORY" name="FG_INQUIRY_HISTORY" <%= ("Y".equals(JspUtils.getDefaultValue(_dl_BoardManage_getTypeDetail, "FG_INQUIRY_HISTORY")))?"checked":"" %> value="Y"/><label class="ub-control label">사용</label>
					<input type="radio" class="ub-control radio" id="FG_INQUIRY_HISTORY" name="FG_INQUIRY_HISTORY" <%= ("N".equals(JspUtils.getDefaultValue(_dl_BoardManage_getTypeDetail, "FG_INQUIRY_HISTORY")))?"checked":"" %> value="N"/><label class="ub-control label">미사용</label>
				</td>	
			</tr>
		</tbody>
	</table>
...
uxl.onLoad(function(){
	...
	// 타 게시판 정보 불러오기 버튼 클릭 이벤트 바인딩
	$('#loadBoardBtn').click(function(event){
		var param = "?ID_BOARD_TYPE=" + $('#ID_BOARD_TYPE').val();
		uxl.openWindow('boardPop', uxl.getScreenURL('UBS0009') + param, {width:720, height:520}, callbackLoadBoard);
	});
...
//게시판선택 콜백
function callbackLoadBoard(returnData){
	var boardId = returnData.ID_BOARD;
	...
	var option = {
	  ...
	  , success:function(result) {
		  	//불러온 게시판의 유형으로 값을 세팅
			var dataSet = result.getDataList(0).rows[0];
			$('#FG_DATA_ITEM[value="'+dataSet.FG_DATA_ITEM+'"]').click();
// 			dataSet.FG_DATA_ITEM_NEW == 'Y'?$('#FG_DATA_ITEM_NEW').attr('checked',true):$('#FG_DATA_ITEM_NEW').attr('checked',false);
// 			dataSet.FG_DATA_ITEM_NEW == 'Y'?$('#FG_DATA_ITEM_FILE').attr('checked',true):$('#FG_DATA_ITEM_FILE').attr('checked',false);
// 			dataSet.FG_DATA_ITEM_NEW == 'Y'?$('#FG_DATA_ITEM_REPLY').attr('checked',true):$('#FG_DATA_ITEM_REPLY').attr('checked',false);
			$('#FG_NOTICE[value="'+dataSet.FG_NOTICE+'"]').click();
			$('#CNT_NOTICE').val(dataSet.CNT_NOTICE);
			$('#FG_FILE[value="'+dataSet.FG_FILE+'"]').click();
			$('#DS_FILE_EXTENSION').val(dataSet.DS_FILE_EXTENSION);
			$('#DS_FILE_VOLUME').val(dataSet.DS_FILE_VOLUME);
			$('#CNT_MAX_FILE').val(dataSet.CNT_MAX_FILE);
			$('#FG_REPLY[value="'+dataSet.FG_REPLY+'"]').click();
			$('#FG_ANSWER[value="'+dataSet.FG_ANSWER+'"]').click();
			$('#FG_PRE_NEXT[value="'+dataSet.FG_PRE_NEXT+'"]').click();
			$('#FG_GOOD_BAD[value="'+dataSet.FG_GOOD_BAD+'"]').click();
// 			$('#FG_EXCEL_DOWN[value="'+dataSet.FG_EXCEL_DOWN+'"]').click();
// 			$('#FG_TEXT_EDITOR[value="'+dataSet.FG_TEXT_EDITOR+'"]').click();
		}
	  , async:true	  
	};
