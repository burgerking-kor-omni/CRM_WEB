...
<%
// 	DataList _dl_detail = ViewUtil.getResult().getDataList("CustManageDAO.getOrderDetailRims");
...
uxl.onLoad(function(){
	...
	options = {
		...
		,colModel:[
		         ...
		         ,{name:'TOT_SALE_AMT',width:15,align:'center',formatter:setComma,sortable:false}
				 ,{name:'SALE_QTY',width:10,align:'center',sortable:false}
				 ,{name:'',width:40,align:'center',sortable:false,hidden:true}
		]
		, height:150
		, autowidth:true
		, onSelectRow: function(rowid){
...
// 	 		uxl.moveLocation(uxl.getScreenURL('CUS0002') ); //+ '?ID_APPR='+selectedRow.ID_APPR + '&ID_ISSUE='+selectedRow.ID_BIZ_KEY);
	...
	// 코드 바인딩
	uxl.bindComboCode('CD_PAY_WAY','CD_PAY_WAY',{emptyOption : '선택안됨'});
...
// 	}
	...
	$('#TOT_SALE_AMT').val($('#TOT_SALE_AMT').val()+' 원');
	$('#TOT_DC_AMT').val($('#TOT_DC_AMT').val()+' 원');
	$('#ACTUAL_SALE_AMT').val($('#ACTUAL_SALE_AMT').val()+' 원');
});
