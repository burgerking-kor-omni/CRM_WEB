...
// 	if( apMaxRole.equals(VocConstantHolder.ROLE_VOC_ADMIN) || apMaxRole.equals(VocConstantHolder.ROLE_ID_ADMIN))
	...
	DataList dataList = ViewUtil.getResult().getDataList("ForecastInquiry.getForecastDetail");
...
uxl.onLoad(function(){
	...
	if("Y" == hasRole){ //관리자일 경우
		...
		$('#DS_YYYY').attr('disabled',true);
		$('#DS_DT_MM').attr('disabled',true);
		$('#DS_MESSAGE').attr('readonly',true);
	...
	$('#inquiryRecord').click(function(){
		uxl.openWindow('deptSearch', uxl.getScreenURL('POP0100')+'?ID_BEFORE_FORECAST='+$('#ID_BEFORE_FORECAST').val(), { width:'960px' ,height:'450px',scrollbars:'yes'});
	});
