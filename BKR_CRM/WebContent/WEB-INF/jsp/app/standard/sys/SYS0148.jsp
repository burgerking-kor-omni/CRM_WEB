...
<tr id="layout-1_2">
<th id="layout-1_2_3"><label class="ub-control label"             for="LOGIN_ID">ID (사번)</label></th>
<td id="layout-1_2_4"><input type=text  class="ub-control input-text " id="LOGIN_ID" name="LOGIN_ID"            metaessential="1"           style="text-align:left;width:120px;"><span class="ub-control button inner icon" id="checkBtn">    <a href="#none" title="Add"><span class="ico-add"></span>중복확인</a></span><label id="dupAlertMsg" name="dupAlertMsg" class="ub-control label txt-color01"></label></td>
<th id="layout-1_2_5"><label class="ub-control label"             for="PASSWD">비밀번호</label></th>
<td id="layout-1_2_6"><input type=password  class="ub-control input-text " id="PASSWD" name="PASSWD"     style="width:120px;"        metaessential="1"><input type=hidden id="CRYPTO_PASSWD" name="CRYPTO_PASSWD"><label class="ub-control label txt-color01">(영문, 숫자 포함 10자리)</label></td>
</tr>
<tr id="layout-1_3">
<th id="layout-1_3_7"><label class="ub-control label"             for="CD_ROLE">사용자권한</label></th>
<td id="layout-1_3_8"><select id="CD_ROLE" name="CD_ROLE" class="ub-control select "     style="width:120px;" size="1"       metaessential="1"></select></td>
<th id="layout-1_3_9"><label class="ub-control label"             for="CD_DUTY">직책</label></th>
<td id="layout-1_3_10"><select id="CD_DUTY" name="CD_DUTY" class="ub-control select "     style="width:120px;" size="1" datacid="CD_DUTY"      metaessential="1"><%    if(!"선택".equals("")){        out.println("<OPTION VALUE=\"\">선택</OPTION>\r\n");    }    if(_dl_CD_DUTY != null){        for(int codeIdx=0; codeIdx < _dl_CD_DUTY.getRowCount(); codeIdx++){            String code = _dl_CD_DUTY.getString(codeIdx, ConstantHolder.CODE_COLUMN_CODE);            String name = _dl_CD_DUTY.getString(codeIdx, ConstantHolder.CODE_COLUMN_NAME);            out.println("<option VALUE=\""+code+"\">"+name+"</option>\r\n");        }    }%></select></td>
</tr>
<tr id="layout-1_4">
<th id="layout-1_4_11"><label class="ub-control label"             for="DEPT_NM">부서</label></th>
<td id="layout-1_4_12" colspan=3><input type=text  class="ub-control input-text " id="DEPT_NM" name="DEPT_NM"            metaessential="1"           style="text-align:left;width:120px;"><input type=hidden id="DEPT_ID" name="DEPT_ID"><a href="#none" title="search" class="ub-control button imgBtn img-search" id="searchDept" name="searchDept"><span>@@</span></a></td>
</tr>
<tr id="layout-1_5">
<th id="layout-1_5_13"><label class="ub-control label"             for="MOBILE_NO">핸드폰 번호</label></th>
<td id="layout-1_5_14"><input type=text  class="ub-control input-text " id="MOBILE_NO" name="MOBILE_NO"            metaessential="1"           style="text-align:left;width:120px;"></td>
<td id="layout-1_5_15" colspan=2></td>
</tr>
<tr id="layout-1_6">
<th id="layout-1_6_16"><label class="ub-control label"             for="EMAIL_DS">이메일</label></th>
<td id="layout-1_6_17"><input type=text  class="ub-control input-text " id="EMAIL_DS" name="EMAIL_DS"            metaessential="1"           style="text-align:left;width:200px;"></td>
<td id="layout-1_6_18" colspan=2></td>
</tr>
<tr id="layout-1_7">
<th id="layout-1_7_19"><label class="ub-control label"             for="INCUMBENT_CD">재직구분</label></th>
<td id="layout-1_7_20"><select id="INCUMBENT_CD" name="INCUMBENT_CD" class="ub-control select "     style="width:120px;" size="1" datacid="INCUMBENT_USER_CD"      metaessential="1"><%    if(!"선택".equals("")){        out.println("<OPTION VALUE=\"\">선택</OPTION>\r\n");    }    if(_dl_INCUMBENT_USER_CD != null){        for(int codeIdx=0; codeIdx < _dl_INCUMBENT_USER_CD.getRowCount(); codeIdx++){            String code = _dl_INCUMBENT_USER_CD.getString(codeIdx, ConstantHolder.CODE_COLUMN_CODE);            String name = _dl_INCUMBENT_USER_CD.getString(codeIdx, ConstantHolder.CODE_COLUMN_NAME);            out.println("<option VALUE=\""+code+"\">"+name+"</option>\r\n");        }    }%></select></td>
<!-- 
<th id="layout-1_7_21"><label class="ub-control label">수관기능</label></th>
<td id="layout-1_7_22"><input class="ub-control input-checkbox" type="checkbox" id="btnfgrecv" name="btnfgrecv"     value="Y"><label></label></input><input type=hidden id="FG_RECV" name="FG_RECV"></td>
-->
<td id="layout-1_7_23" colspan=2></td>
</tr>
</tbody>
</table>
...
uxl.onLoad(function(){
	...
	uxl.bindComboCode('CD_ROLE', 'CD_ROLE', {parameter : {}, emptyOption:'선택'});
...
function fnRegist(){
	...
	var option = {
	  ...
	  , success:function(result) {
		  	...
		  	uxl.moveLocation(uxl.getScreenURL('SYS0021'));
		}
	  , async:true
	  , message : {
