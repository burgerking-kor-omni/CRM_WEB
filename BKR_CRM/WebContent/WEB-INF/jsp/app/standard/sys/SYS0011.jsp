 ...
 *  버    전 :  1.0
 *  기타사항 :
 *    2016-05-04 : Wide 메뉴 추가로 인한 메뉴유형에 단구분자 추가
		...
		<div class="ub-layout leftBox"   style="width:30%;">
			<h2 class="ub-control title">메뉴구조
				<div class="ub-layout button inner"  style="text-align:right;">
					...
					<select id="MENU_STRUCTURE" name="MENU_STRUCTURE" class="ub-control select " style="width:120px;" size="1" datacid="MENU_STRUCTURE">
					<%	if(!"".equals("")){
						...
						if(_dl_TP_MENU_STRUCTURE != null){
							for(int codeIdx=0; codeIdx < _dl_TP_MENU_STRUCTURE.getRowCount(); codeIdx++){
								...
								String selected = code.equals('N')?"selected":"";
		...
		<div class="ub-layout rightBox"   style="width:69%;">
			...
			<form id="detailForm" name="detailForm">
				<table class="ub-control table normal ">
					...
					<td id="layout-1_1_4"><input type=text  class="ub-control input-text " id="MENU_NAME" name="MENU_NAME"            metaessential="1"           style="text-align:left;width:150px;"></td>
					</tr>
					<tr id="layout-1_2">
					<th id="layout-1_2_5"><label class="ub-control label"             for="MENU_TYPE">메뉴유형</label></th>
					<td id="layout-1_2_6"><select id="MENU_TYPE" name="MENU_TYPE" class="ub-control select "     style="width:150px;" size="1" datacid="MENU_TYPE"N'"     metaessential="1"><%    if(!"".equals("")){        out.println("<OPTION VALUE=\"\"></OPTION>\r\n");    }    if(_dl_MENU_TYPE != null){        for(int codeIdx=0; codeIdx < _dl_MENU_TYPE.getRowCount(); codeIdx++){            String code = _dl_MENU_TYPE.getString(codeIdx, ConstantHolder.CODE_COLUMN_CODE);            String name = _dl_MENU_TYPE.getString(codeIdx, ConstantHolder.CODE_COLUMN_NAME);            String selected = code.equals('N')?"selected":"";            out.println("<option VALUE=\""+code+"\" "+selected+">"+name+"</option>\r\n");        }    }%></select></td>
					<th id="layout-1_2_7"><label class="ub-control label"             for="PARENT_MENU_ID">상위메뉴</label></th>
					<td id="layout-1_2_8"><select id="PARENT_MENU_ID" name="PARENT_MENU_ID" class="ub-control select "     style="width:150px;" size="1"></select></td>
					</tr>
					<tr id="layout-1_3">
	...
	var option = {
			...
			, success:function(result){
				...
				if(node.data.key=="New-Menu"){
					...
					combo.emptySelect("");
					combo.addOption('ROOT' ,'');
					for( var i=0 ; i < rowCount ;  i++){
						combo.addOption(dataSet[i].MENU_NAME ,dataSet[i].MENU_ID);
					}
					$('#PARENT_MENU_ID option:eq(0)').remove();
					combo.val(parentId).change();
				}
