...
<!-- layout-1 Start -->
<h1 class="ub-control title">게시판 템플릿 마스터관리 목록</h1>
...
<tbody>
	<tr id="layout-1_1">
		<th id="layout-1_1_1"><label class="ub-control label">게시판템플릿ID</label></th>
		<td id="layout-1_1_2"><input type=text  class="ub-control input-text " id="ID_BOARD_TYPE_MASTER" name="ID_BOARD_TYPE_MASTER" value="${paramMap.ID_BOARD_TYPE_MASTER}" style="text-align:left;width:100px;"></td>
	</tr>
	<tr id="layout-1_2">
		<th id="layout-1_2_3"><label class="ub-control label">게시판템플릿명</label></th>
		<td id="layout-1_2_4"><input type=text  class="ub-control input-text " id="NM_BOARD_TYPE_MASTER" name="NM_BOARD_TYPE_MASTER" value="${paramMap.NM_BOARD_TYPE_MASTER}" style="text-align:left;width:400px;"></td>
	</tr>
</tbody>
</table>
...
uxl.onLoad(function(){
	...
	var options = {
		//header name
		colNames:['게시판템플릿ID', '게시판템플릿명', '등록자ID', '등록자', '등록일']
		
		//mapping column
		, colModel:[
			    {name:'ID_BOARD_TYPE_MASTER',       width:30, align:'center'}
			  , {name:'NM_BOARD_TYPE_MASTER',       width:80, align:'left'}
		...
		, onSelectRow: function(rowid){
			...
			uxl.moveLocation(uxl.getScreenURL('SYS0304') + param);
		}
	};
	...
	$('#registBtn').click(function(){
		uxl.moveLocation(uxl.getScreenURL('SYS0303'));
	});
	...
	$('#boardCloneBtn').click(function(){
		...
		if (uxl.isNull(boardId)) {
			...
			//복제창호출	
			uxl.openWindow('cloneBoardPop', uxl.getScreenURL('SYS0302') + "?boardId=" + boardId, {width:'320px', height:'170px'}, callbackClone);
		}
	});
...
function setLinkBoardBtn(cellValue, opts, rowObject) {
	...
	html += "<a href='javascript:fnMoveBoard(\"" + rowObject.ID_BOARD + "\");' title='바로가기'>";
	html += "바로가기</a></span>";
...
//게시판 바로가기
function fnMoveBoard(boardId){
	uxl.openWindow('boardPop', uxl.getScreenURL('UBD1001') + "?boardId=" + boardId, {width:'1000px', height:'700px'});
}
