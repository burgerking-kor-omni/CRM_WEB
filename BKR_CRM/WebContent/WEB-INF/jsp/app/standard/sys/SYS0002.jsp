...
<td id="layout-1_2_8"><select id="CODE_TP" name="CODE_TP" class="ub-control select "     style="width:120px;" size="1" datacid="CODE_TP" defaultvalue="${paramMap.CODE_TP}"><%    if(!"전체".equals("")){        out.println("<OPTION VALUE=\"\">전체</OPTION>\r\n");    }    if(_dl_CODE_TP != null){        for(int codeIdx=0; codeIdx < _dl_CODE_TP.getRowCount(); codeIdx++){            String code = _dl_CODE_TP.getString(codeIdx, ConstantHolder.CODE_COLUMN_CODE);            String name = _dl_CODE_TP.getString(codeIdx, ConstantHolder.CODE_COLUMN_NAME);            String selected = code.equals(ViewUtil.getParameter().getValue("CODE_TP"))?"selected":"";            out.println("<option VALUE=\""+code+"\" "+selected+">"+name+"</option>\r\n");        }    }%></select></td>
</tr>
</tbody>
</table>
	...
	*/
	var options = {
		...
		, onSelectRow: function(rowid){
			...
			uxl.moveLocation(uxl.getScreenURL('SYS0004') + '?CODE_CATEGORY_ID='+selectedRow.CODE_CATEGORY_ID);
		}
        ,height: 435
	};
