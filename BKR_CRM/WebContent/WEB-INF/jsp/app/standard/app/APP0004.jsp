...
<div class="ub-layout treeBox" id="treeList" name="treeList"   style="height:390px;"><div class="ub-control temp" style="border:2px solid gray;width:100px;height:100px;text-align:center;"><h3 style="padding-top:20px;">Tree Control</h3></div>
...
<td id="layout-1_1_2"><input type=text  class="ub-control input-text " id="NM_USER" name="NM_USER"       value="${paramMap.NM_USER}"     metaessential="1"             style="ime-mode:active;text-align:left;width:120px;"><input type=hidden id="DEPT_ID_REAL" name="DEPT_ID_REAL"><a href="#none" title="search" class="ub-control button imgBtn img-search" id="fnSearch" name="fnSearch"><span>@@</span></a></td>
</tr>
</tbody>
</table>
...
<h2 class="ub-control title">결재자</h2>
<div class="ub-layout list"><div class="ub-control temp" style="border:2px solid gray;width:100%;height:300px;text-align:center;"><h3 style="padding-top:120px;">Grid Control</h3></div>
...
<span class="ub-control button inner icon" id="close">
...
uxl.onLoad(function() {
	...
	var treeOption = {
 	...
 	var options = {
		...
		//mapping column
		,colModel:[
		           {name:'DUTY_CD',align:'center',width:30,hidden:true}
		          ,{name:'DUTY_NM',align:'center', width:30}
		          ,{name:'USER_ID',width:120,hidden:true}
		          ,{name:'USER_NM',align:'center',width:30}
		          ,{name:'DEPT_NM',width:30}
		          ,{name:'DEPT_ID_REAL',align:'center',width:40}
		         ]
		,autowidth:true
		,height:108/*
	...
	//결재선직원목록 JqGrid (RightBox)
 	var options = {
		...
		//mapping column
		,colModel:[
				   {name:'TP_APPR_LINE' ,width:0, hidden:true}
				  ,{name:'TP_APPR_LINE_NM',align:'center',width:29, hidden:true}
		           /* {name:'TP_APPR_LINE' ,width:30,editable: true,formatter: 'select',edittype: 'select',editoptions: {value: {'01': '결재','02': '협의'}}} */
		          ,{name:'ID_DEPT',width:30,hidden:true}
		          ,{name:'ID_ORDER',width:29,align:'center',sortable: false}
		          ,{name:'NM_DEPT',width:40}
		          ,{name:'DUTY_CD',width:0,hidden:true}
		          ,{name:'DUTY_NM',align:'center',width:30}
		          ,{name:'ID_EMP',width:40,hidden:true}
		          ,{name:'NM_EMP',align:'center',width:30}
	...
	***************************************/
	...
	$('#NM_USER').keydown(function(event) {
 	...
 	// 직원검색
	if($('#NM_USER').val()!='' || $('#NM_USER').val()!=null){
		fnSearch();
	} 
...
function fnSave(){
	...
	//선택한 결재자가 이미 등록되 있는지 확인한다.
	for(var i=0; i<idEmp.length; i++){ 
		for(var j=0; j<parentIdEmp.length; j++){ 
			if(idEmp[i] == parentIdEmp[j]){ 
	       		alert("동일한 결재자를 추가할 수  없습니다.");
	       		return;
	   		} 
	   	}
