...
<td id="layout-1_1_2" colspan=7><select id="CD_BRANCH" name="CD_BRANCH" class="ub-control select "     style="width:120px;" size="1" datacid="CD_BRANCH" defaultvalue="${paramMap.CD_BRANCH}"><%    if(!"선택".equals("")){        out.println("<OPTION VALUE=\"\">선택</OPTION>\r\n");    }    if(_dl_CD_BRANCH != null){        for(int codeIdx=0; codeIdx < _dl_CD_BRANCH.getRowCount(); codeIdx++){            String code = _dl_CD_BRANCH.getString(codeIdx, ConstantHolder.CODE_COLUMN_CODE);            String name = _dl_CD_BRANCH.getString(codeIdx, ConstantHolder.CODE_COLUMN_NAME);            String selected = code.equals(ViewUtil.getParameter().getValue("CD_BRANCH"))?"selected":"";            out.println("<option VALUE=\""+code+"\" "+selected+">"+name+"</option>\r\n");        }    }%></select><select id="CD_TEAM" name="CD_TEAM" class="ub-control select "     style="width:120px;" size="1"  defaultvalue="${paramMap.CD_TEAM}"></select></td>
</tr>
<tr id="layout-1_2">
<th id="layout-1_2_3"><label class="ub-control label">재직여부</label></th>
<td id="layout-1_2_4"><select id="INCUMBENT_CD" name="INCUMBENT_CD" class="ub-control select "     style="width:120px;" size="1" datacid="INCUMBENT_USER_CD" defaultvalue="${paramMap.INCUMBENT_CD}"><%    if(!"선택".equals("")){        out.println("<OPTION VALUE=\"\">선택</OPTION>\r\n");    }    if(_dl_INCUMBENT_USER_CD != null){        for(int codeIdx=0; codeIdx < _dl_INCUMBENT_USER_CD.getRowCount(); codeIdx++){            String code = _dl_INCUMBENT_USER_CD.getString(codeIdx, ConstantHolder.CODE_COLUMN_CODE);            String name = _dl_INCUMBENT_USER_CD.getString(codeIdx, ConstantHolder.CODE_COLUMN_NAME);            String selected = code.equals(ViewUtil.getParameter().getValue("INCUMBENT_CD"))?"selected":"";            out.println("<option VALUE=\""+code+"\" "+selected+">"+name+"</option>\r\n");        }    }%></select></td>
...
//	$('#CD_BRANCH').attr('disabled', true);
	uxl.bindComboCode('COMPANY_CD', 'CD_ORG_COMP_1', {parameter : {USER_ID:'${user.id}'}, emptyOption:'전체'});
	
//	uxl.chainCombo($('#CD_BRANCH'), $('#CD_TEAM') , 'CD_TEAM', {parameter : {PARENT_CODE:'${user.cdBranch}'}});
	uxl.chainCombo($('#CD_BRANCH'), $('#CD_TEAM') , 'CD_TEAM', {parameter : {PARENT_CODE:$('#CD_BRANCH').val()}});
